...
root = tk.Tk()
root.title('交换机自动巡检 V1.0.0')
root.geometry('800x500')

information = tk.Text(root, width=50, height=30)
information.grid(row=0, column=1, padx=10, pady=10, rowspan=10, columnspan=2)
        ...
        try:
            with ConnectHandler(**dev) as conn:
                ...
                if 'huawei' in dev['device_type']:
                    ...
                    information.insert(tk.END, "error")
                with open('./conf_bak/' + dev['host'] + '_conf_bak.txt', mode='w', encoding='utf8') as f:
                    information.insert(tk.END, '正在备份:' + dev['host'])
                    # 文件读写异常处理
                    try:
                        f.write(output)
                    except PermissionError:
                        information.insert(tk.END, "*****-无写入权限，请将文件夹赋予读写权限-*****")
                        continue
                    else:
                        information.insert(tk.END, dev['host']+'配置文件备份成功！')
        # 连接异常处理
            ...
            with ConnectHandler(**dev) as conn:
                ...
                with open('./log/' + dev['host'] + '_conf_log.txt', mode='w', encoding='utf8') as f:
                    ...
                    try:
                        ...
                        information.insert(tk.END, dev['host']+'log文件备份成功！')
        # 连接异常处理
        except exceptions.NetmikoAuthenticationException:
            information.insert(tk.END, '\n**********' + dev['host'] + '：登录验证失败！**********')
            failed_ips.append(dev['host'])
    ...
    try:
        for x in range(len(devices)):
            ...
            with ConnectHandler(**devices[x]) as conn:
                conn.enable()
                information.insert(tk.END, '正在巡检：' + devices[x]['host'] + ' ...')
        ...
        if each['device_type'] == 'huawei':
            ...
            for res in checkres:
                ...
                with open('./huaweixunjian/' + res[0] + '.txt', mode='w', encoding='utf8') as f:
                    information.insert(tk.END, '正在写入巡检结果:' + res[0])
                    # 文件读写异常处理
                    try:
                        f.write(output_all)
                    except PermissionError:
                        ...
                        information.insert(tk.END, '写入成功！')
...
def main():
    information.insert(tk.END, "##############################################\n")
    information.insert(tk.END, "批量备份交换机配置请按 配置和日志\n")
    information.insert(tk.END, "批量巡检交换机设备请按 自动巡检\n")
    information.insert(tk.END, "退出称序请按 退出\n")



information.insert(tk.END, "欢迎光临！")
tk.Button(root, text="开始使用", width=15, command=main).grid(row=0, column=0, padx=10, pady=10)
tk.Button(root, text="配置和日志", width=15, command=configlog).grid(row=1, column=0, padx=10, pady=10)
tk.Button(root, text="自动巡检", width=15, command=xunjian).grid(row=2, column=0, padx=10, pady=10)
tk.Button(root, text="备用（无）", width=15, command='').grid(row=3, column=0, padx=10, pady=10)
tk.Button(root, text="退出程序", width=15, command=root.quit).grid(row=4, column=0, padx=10, pady=10)
